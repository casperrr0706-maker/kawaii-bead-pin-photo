...
2. `references/shiny-sample.png` — TEXTURE + GRID + COMPOSITION anchor (**abstract macaron square-stud sample, user-confirmed in THIS conversation**: a silver safety pin across the top, one independent small jump ring per column hooked to the pin, a completely flat square-stud panel of abstract macaron color blocks (pink/blue/white/lavender/mint) with distinct square beads and gaps — built from the approved dolphin finished sample with ONLY the panel pattern replaced by abstract color blocks, so bead size/count/placement match the approved dolphin panel exactly — a pearl/star/drop charm cluster at the right end, dreamy beach-girl background; NO concrete subject, NO animal, NO face, NO black eyes). The anchor locks texture, bead shape/size/count/placement (the panel grid density is copied from this anchor) and the fixed composition framework WITHOUT contaminating the new subject's colors; all subject color comes only from the user's photo mosaic.
