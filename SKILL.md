...
2. `references/shiny-sample.png` — TEXTURE + GRID anchor (the user-approved **ALL-WHITE/transparent square rhinestone-studded sample**: blank white/clear square-cut crystal studs in a regular square grid, NO color, NO black eyes, NO animal/face details, fixed bead size/count/placement, full dreamy composition). The panel beads themselves are WHITE/clear and carry NO color, so they cannot contaminate the new subject's colors — all color comes only from the user's photo mosaic.
