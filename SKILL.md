...
2. `references/shiny-sample.png` — TEXTURE + GRID + COMPOSITION anchor (**clean white square-rhinestone stud sample, user-confirmed in THIS conversation**: a silver safety pin across the top, one independent small jump ring per column hooked to the pin, a completely flat panel of PURE WHITE square-cut rhinestone studs in a regular grid — no pattern, no face, no eyes, no expression, no colored beads — with a single NEUTRAL round clear-cut diamond ornament on the pin bar center (position/texture display ONLY — never copy its shape) and dreamy sky-blue layered background (back layer sky/clouds, mid layer pin+panel, front layer bokeh glows, blurred toy silhouette at bottom right) and generic pearl/drop/tassel charms at the right end; NO subject, NO animal, NO face, NO black eyes, NO transparent orb, NO beach/sand/flower/toy as foreground subject, NO watermark). The anchor locks texture, bead shape/size/count/placement (the panel grid density is copied from this anchor), the layered background composition, and the fixed framework WITHOUT contaminating the new subject; all subject color comes only from the user's photo mosaic, and scene elements are specified by prompt only. **The pin ornament in the anchor is a NEUTRAL placeholder — its shape/color is never copied; the real pin ornament is always extracted from the CURRENT subject by prompt.**
...
**PIXEL-ART / FLAT-BLOCK FAILURE DIAGNOSIS (mandatory self-check)**: if the generated final image reads as PIXEL ART / flat color squares / plain matte blocks — beads with NO sparkle, NO mirror highlights, NO crystal facets, flat pixel squares instead of square-cut rhinestone studs — that is the signature that the texture anchor (`references/shiny-sample.png`) was NOT passed to the image tool or the sparkle wording was dropped. Re-run the generation IMMEDIATELY with BOTH references passed together (`bead_pattern.png` + `shiny-sample.png`) and the full Prompt Shape below (square rhinestone-studded, mirror highlight, facet star glints, distinct beads with gaps). Never deliver a pixel-art-looking final image. (This exact failure occurred in a fresh install: output was flat pixel blocks with no rhinestone texture.)
...
- **COMPLETELY FLAT, NOTHING RAISED (user-confirmed)**: every square stud is a FLAT pasted tile lying on the same plane as the panel — NO raised/domed beads, NO 3D bumps, NO embossed relief, NO pop-out studs, NO depth illusion. The only variation is bead-surface shine (mirror highlight / star glints); the beads themselves are flat tiles, never domed cabochons.
...
- **Bead texture — SQUARE CUT RHINESTONE-STUDDED MOSAIC, STRONG SPARKLE (final output standard, user-approved)**: every bead in the panel is a **square** rhinestone / crystal stud — square-cut flat crystal tiles in a regular square grid, like square mosaic tiles, NOT round beads, NOT oval, NOT hexagonal. **GRID DENSITY FOLLOWS THE ANCHOR (user-confirmed in THIS conversation): the panel's bead size, row/column count, and grid placement MUST match `references/shiny-sample.png` exactly** (the anchor was built from the approved dolphin finished sample with only the panel pattern replaced by abstract color blocks, so its bead density IS the approved density; never fewer, never denser, never a different row/column count). **DISTINCT BEADS, NEVER FUSED (user-confirmed)**: every bead must be clearly SEPARATED from its neighbors — each stud is an individual piece with visible small gaps/grout lines between beads, like separate rhinestones pasted onto a base with backing showing between them. Beads must NEVER fuse into one continuous sheet, NEVER look like a seamless glued-tile surface, NEVER merge into a single block. **Strong sparkle (user-confirmed)**: every square stud carries a bright mirror highlight AND facet-like star glints; the whole panel glitters like a diamond-studded mosaic, brighter and more luminous than a plain sample. This is the FIXED final texture for every output. Reference: `references/shiny-sample.png` (clean white square-rhinestone stud panel — silver pin, per-column rings, pure-white bead grid, layered dreamy sky-blue background, generic pearl/drop/tassel charms; no animal/face/expression/colored beads; the bead density and layered composition are copied from the anchor). The panel stays completely flat; shine is bead-surface glitter, never 3D relief. Aligned rows/columns with slight natural sag.
...
- **Pin decoration (no bead strings on the pin)**: the pin itself carries NO string of beads. It carries exactly ONE small themed ornament **extracted from the main subject** — e.g. for a horse subject, a tiny plastic horse bead / horse-shaped charm on the pin bar; for a dolphin, a small dolphin/drop; for a cake, a mini cake bead; for a strawberry-kitty subject, a small strawberry bead. The ornament is a single item, uses subject colors, and never becomes a bead string or a row of decorations. **Anchor placeholder rule (user-confirmed)**: the ornament shown in the texture anchor (`shiny-sample.png`) is a NEUTRAL round clear-cut diamond used ONLY to display position and texture — its shape/color is NEVER copied into outputs; every real pin ornament must be extracted from the CURRENT subject and specified by prompt (horse → tiny horse bead, rabbit → tiny rabbit bead, kitty → tiny kitty/strawberry bead). **Anti-contamination (user-confirmed)**: the pin ornament must NOT be a transparent round ball containing an animal (e.g. the texture anchor's dolphin must not be carried over — the pin ornament always comes from the CURRENT subject, never from the reference sample's old subject).
...
The bead panel is a COMPLETELY FLAT regular grid of SQUARE RHINESTONE-STUDDED beads that faithfully reproduces the provided mosaic/pixel plate — the FULL image including its background colors: same simplified color blocks, same silhouette, same background color as the panel base/frame, nothing more. **The panel's bead size, row/column count and grid placement match the style reference's bead grid exactly (same density as the reference sample — never fewer, never denser).** Every bead is a square-cut crystal stud — square, straight-edged, checkerboard-like regular grid, NOT round, NOT oval, NOT hexagonal (not a single bead may be round) — with a bright mirror highlight AND facet-like star glints, like a diamond-studded mosaic catching light, brighter and more luminous; beads match the reference grid's size, count and placement; aligned rows and columns with slight natural sag. **Every bead is a DISTINCT SEPARATE stud: each bead is clearly separated from its neighbors with visible small gaps/grout lines between them, like individual rhinestones pasted on a base with backing visible between beads — NEVER fused into one continuous sheet, NEVER a seamless glued-tile surface, NEVER merged into one block.** Every stud is a FLAT tile on the same plane as the panel — NO raised/domed beads, NO 3D bumps, NO embossed relief, NO pop-out studs, NO depth illusion; only bead-surface shine varies. The pattern appears ONLY through the color of each bead, like a pixel-art mosaic; every bead is on one plane. NO 3D relief, no raised figure, no volumetric character or pet built on top of the beads. Do NOT draw eyes, flowers, or any fine detail on the beads; no letters or numbers anywhere.
