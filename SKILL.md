...
2. `references/shiny-sample.png` — TEXTURE + GRID + COMPOSITION anchor (**deterministically generated 22 ROWS × 18 COLUMNS abstract macaron square-stud sample**: a silver safety pin across the top, one independent small jump ring per column hooked to the pin (18 rings), a completely flat 22×18 square-stud panel of abstract macaron color blocks (pink/blue/lavender/mint/white/yellow) with distinct beads and gaps, a pearl/star/drop charm cluster at the right end, dreamy light gradient background; NO concrete subject, NO animal, NO face, NO black eyes). **This anchor was replaced (user-confirmed) because the previous sample's panel was NOT 22×18 — the image model visually copies the anchor's grid density, so a sparse anchor produced sparse outputs (11×11). The 22×18 grid is now deterministically built into the anchor itself.** The anchor locks texture, bead shape/size/count/placement (22 rows × 18 columns) and the fixed composition framework WITHOUT contaminating the new subject's colors; all subject color comes only from the user's photo mosaic.
...
- **Bead texture — SQUARE CUT RHINESTONE-STUDDED MOSAIC, STRONG SPARKLE (final output standard, user-approved)**: every bead in the panel is a **square** rhinestone / crystal stud — square-cut flat crystal tiles in a regular square grid, like square mosaic tiles, NOT round beads, NOT oval, NOT hexagonal. **GRID COUNT HARD-LOCKED (user-confirmed): the panel is EXACTLY 22 ROWS × 18 COLUMNS of square studs** — the same bead count as `references/shiny-sample.png`; never fewer, never denser, never a different row/column count. **Bead size, count, and grid placement MUST match `references/shiny-sample.png` exactly** (22 rows × 18 columns, same bead proportions, same panel position). **DISTINCT BEADS, NEVER FUSED (user-confirmed)**: every bead must be clearly SEPARATED from its neighbors — each stud is an individual piece with visible small gaps/grout lines between beads, like separate rhinestones pasted onto a base with backing showing between them. Beads must NEVER fuse into one continuous sheet, NEVER look like a seamless glued-tile surface, NEVER merge into a single block. **Strong sparkle (user-confirmed)**: every square stud carries a bright mirror highlight AND facet-like star glints; the whole panel glitters like a diamond-studded mosaic, brighter and more luminous than a plain sample. This is the FIXED final texture for every output. Reference: `references/shiny-sample.png` (deterministic 22×18 abstract macaron square-stud sample — silver pin, 18 per-column rings, abstract color blocks, no animal/face; the 22×18 grid is built into the anchor so the model copies the correct density). The panel stays completely flat; shine is bead-surface glitter, never 3D relief. Aligned rows/columns with slight natural sag.
