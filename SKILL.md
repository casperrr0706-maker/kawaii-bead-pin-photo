...
2. `references/shiny-sample.png` — TEXTURE + GRID + STRUCTURE anchor (the user-approved **WHITE PEARL BEAD CURTAIN** sample: white pearl/crystal beads hanging as separate vertical bead-curtain columns from the pin, one independent metal hook per column, gaps between columns showing the background, fixed bead size/count/placement, full dreamy composition). The panel beads themselves are WHITE — their color comes from the user's photo mosaic, never from the reference.
Never generate from the mosaic alone, never from text alone, never swap the texture reference for another image. The texture anchor is what produces the pearl/bead-curtain look and the column structure; without it the model defaults to plain matte beads or a solid connected panel (this exact failure happened once in a fresh install). The anchor locks texture, bead shape/size/count/placement, per-column hanging structure and composition; the panel beads carry NO color of their own, so color contamination is impossible.
...
- **Bead texture — WHITE PEARL BEAD CURTAIN, SEPARATE COLUMNS (final output standard, user-approved)**: the panel is a **white pearl / white crystal bead curtain** — beads hang as **separate vertical bead-curtain columns**, one column one string, NOT a solid connected panel. **Every column is an independent string with visible gaps between columns showing the background.** Each column hangs from its own small metal hook on the pin. Each bead has a bright mirror highlight and sparkling glints; colors are applied from the mosaic (the beads themselves start white). **Bead size, count, and column placement MUST match `references/shiny-sample.png` exactly** (same number of columns, same beads per column, same bead proportions, same panel position) — the anchor fixes the grid spec so the model cannot invent a different bead density, a different number of columns, or merge the columns into a solid panel. This is the FIXED final texture for every output. Reference: `references/shiny-sample.png` (user-approved WHITE pearl bead-curtain sample) and `references/approved-sample.png`. The panel stays completely flat; shine is bead-surface glitter, never 3D relief. Aligned rows/columns with slight natural sag.
...
- **Charms and background (scene follows subject, composition fixed)**: charms and background vary with the subject's natural context — the BACKGROUND SCENE is inferred from what the subject is: a dolphin/sea creature → ocean/beach in blue tones; flowers/plants → forest/garden in green tones; a cake/dessert → pastel dessert-shop/summer cafe setting; a horse → meadow/pasture; a pet → cozy home garden. The COMPOSITION FRAMEWORK is FIXED for every output: dreamy soft-focus, bright and cute, gentle bokeh sparkles, shallow depth of field, square centered framing, optional blurred cute toy silhouette at the bottom right, no readable text. The larger right-end charm and the pin ornament both derive from the subject; the small accents are generic. All accent colors drawn from the subject palette.
...
- Beads form aligned rows/columns; **white pearl bead-curtain texture with SEPARATE COLUMNS** — every bead is a white pearl/crystal bead with bright mirror highlights, columns are independent strings with gaps between them showing the background, matching the reference column count/bead size/placement (see `references/shiny-sample.png`).
...
- The background scene matches the subject's natural context (sea for dolphin, forest for flowers, etc.) with the fixed dreamy composition framework, and does not reduce readability.
...
```text
Create a square kawaii product photo of a handmade bead charm hanging from a shiny silver safety pin. Match the WHITE PEARL BEAD CURTAIN texture AND the column structure (separate vertical bead strings, one independent hook per column, gaps between columns) AND the bead size/count/placement of the provided style reference (NOT plain matte beads, NOT a solid connected bead panel, NOT a different bead count or density).
...
The bead panel is a COMPLETELY FLAT curtain of WHITE PEARL/CRYSTAL BEADS arranged as SEPARATE VERTICAL COLUMNS — each column is an independent bead string, columns do NOT touch or merge into a solid panel, and the background shows through the gaps between columns. The panel faithfully reproduces the provided mosaic/pixel plate — the FULL image including its background colors: same simplified color blocks, same silhouette, same background color as the panel base, nothing more. The pattern appears ONLY through the color of each bead, like a pixel-art mosaic; every bead is on one plane. NO 3D relief, no raised figure, no volumetric character or pet built on top of the beads. Do NOT draw eyes, flowers, or any fine detail on the beads; no letters or numbers anywhere. Every bead is a white pearl/crystal bead with a bright mirror highlight and sparkle, like a pearl curtain catching light; beads match the reference's column count, bead size and placement; aligned columns with slight natural sag.
