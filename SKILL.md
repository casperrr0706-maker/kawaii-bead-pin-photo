...
2. `references/shiny-sample.png` — TEXTURE + GRID anchor (the user-approved **abstract pastel color-block square-stud sample**: square-cut crystal studs in a regular square grid forming abstract color blocks in pink/light-blue/white/lavender/mint — NO concrete subject, NO animal, NO face, NO black eyes, fixed bead size/count/placement, star pin ornament, geometric charms, dreamy beach composition). The anchor carries no recognizable subject, so it locks texture, bead shape/size/count/placement and composition WITHOUT contaminating the new subject; all subject color comes only from the user's photo mosaic.
