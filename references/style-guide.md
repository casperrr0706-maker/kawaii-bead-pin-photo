...
This reference defines the target look for the bead safety-pin photo workflow. The approved anchor lives in this folder: `shiny-sample.png` — an **abstract macaron square-stud sample (user-confirmed in THIS conversation)**: silver safety pin across the top, one independent small jump ring per column hooked to the pin, a completely flat square-stud panel of abstract macaron color blocks (pink/blue/white/lavender/mint) with distinct square beads and gaps — built from the approved dolphin finished sample with ONLY the panel pattern replaced by abstract color blocks, so bead size/count/placement match the approved dolphin panel exactly — pearl/star/drop charms at the right end, dreamy beach-girl background. NO concrete subject, NO animal, NO face, NO black eyes — so it cannot contaminate a new subject. The anchor locks texture, bead shape/size/count/placement (the panel grid density is copied from this anchor) and the fixed composition framework; all subject color comes only from the user's photo mosaic.
