...
This reference defines the target look for the bead safety-pin photo workflow. The approved mood samples live in this folder: `shiny-sample.png` (user-approved **ALL-WHITE/transparent square rhinestone-studded sample** — blank white/clear square-cut crystal studs in a regular square grid, NO color, NO black eyes, NO animal/face details, fixed bead size/count/placement, dreamy beach composition; the beads carry no color so they cannot contaminate the new subject) and `approved-sample.png` (beach-side summer scene with a bead mosaic panel, small flower on the pin, and shell/flower/star charms).
