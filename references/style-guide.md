...
This reference defines the target look for the bead safety-pin photo workflow. The approved anchor lives in this folder: `shiny-sample.png` — a **deterministically generated 22 ROWS × 18 COLUMNS abstract macaron square-stud sample**: silver safety pin across the top, one independent small jump ring per column (18 rings) hooked to the pin, a completely flat 22×18 square-stud panel of abstract macaron color blocks (pink/blue/lavender/mint/white/yellow) with distinct beads and gaps, pearl/star/drop charms at the right end, dreamy light gradient background. NO concrete subject, NO animal, NO face, NO black eyes — so it cannot contaminate a new subject. **It replaced the previous sample because the image model visually copies the anchor's grid density: the old anchor's panel was not 22×18 and produced sparse outputs (11×11). The 22×18 grid is deterministically built into the anchor itself.** The anchor locks texture, bead shape/size/count/placement and the fixed composition framework; all subject color comes only from the user's photo mosaic.
...
- **Beads are SQUARE CUT RHINESTONE-STUDDED, STRONG SPARKLE, DISTINCT, EXACTLY 22 ROWS × 18 COLUMNS (final output standard, user-approved)**: every bead is a **square** rhinestone/crystal stud — square-cut flat crystal tiles in a regular square grid, like square mosaic tiles. NEVER round, oval, or hexagonal beads. **The panel has EXACTLY 22 ROWS × 18 COLUMNS of square studs — the same bead count as `shiny-sample.png`; never fewer, never denser.** **Every bead is clearly SEPARATED from its neighbors with visible small gaps/grout lines between them, like individual rhinestones pasted on a base with backing visible between beads — NEVER fused into a continuous sheet, NEVER a seamless glued-tile surface.** **Every stud is a FLAT tile on the same plane — NO raised/domed beads, NO 3D bumps, NO relief, NO depth.** Each bead: bright mirror highlight AND facet-like star glints, saturated luminous color, whole panel glitters like a diamond-studded mosaic, brighter and more luminous. **Bead size / count / grid placement MUST match `shiny-sample.png` exactly** (22 rows × 18 columns, same bead proportions, same panel position) — the anchor is the deterministically generated 22×18 abstract macaron square-stud sample (silver pin, 18 per-column rings, abstract color blocks, no animal/face), so it locks texture, shape, and grid spec WITHOUT contaminating the new subject's identity; all subject color comes only from the user's photo mosaic. Reference: `shiny-sample.png`.
